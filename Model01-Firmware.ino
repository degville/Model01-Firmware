...
// GM: Modifications made to:
//     - physically swap ctrl and shift keys over (both sides)
//     - change mouse keys (esdf)
//     - modify mouse movement
//     - add mouse fast and slow modifiers (shift/cmd respectively)
...
KEYMAPS(
  ...
  (___,          Key_1, Key_2, Key_3, Key_4, Key_5, Key_LEDEffectNext,
   ...
   Key_LeftShift, Key_Backspace, Key_LeftGui, Key_LeftControl, // GM: Left Control/Shift swap
   ShiftToLayer(FUNCTION),

   M(MACRO_ANY),  Key_6, Key_7, Key_8,     Key_9,         Key_0,         LockLayer(NUMPAD),
   Key_Enter,     Key_Y, Key_U, Key_I,     Key_O,         Key_P,         Key_Equals,
                  Key_H, Key_J, Key_K,     Key_L,         Key_Semicolon, Key_Quote,
   LCTRL(Key_D),  Key_N, Key_M, Key_Comma, Key_Period,    Key_Slash,     Key_Minus, // GM: Ctrl D for Hangouts mute
   Key_RightControl, Key_LeftAlt, Key_Spacebar, Key_RightShift, // GM: Right Control/Shift swap
   ShiftToLayer(FUNCTION)),
...
const macro_t *macroAction(uint8_t macroIndex, uint8_t keyState) {
  ...
  case MACRO_VERSION_INFO:
    ...
    // GM: add toggles for mouse speed in mouse mode
    case M_MOUSE_SLOW:
      if (keyIsPressed(keyState)) {
        MouseKeys.speed = 3;
